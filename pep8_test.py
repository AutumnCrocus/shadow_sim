...
def demo_game_play(Player1, Player2, D1, D2, win, lose, lib_num, virtual_flg=False, deck_name_list=None,
    ...
    if history_flg:
        (w, l, lib, turn, history) = G.start(
            f, virtual_flg=virtual_flg, history_flg=history_flg)
    ...
    if not virtual_flg:
        mylogger.info("Game end")
        mylogger.info("Player1 life:{} Player2 life:{}".format(
            f.players[0].life, f.players[1].life))
        f.show_field()
    player1_win_turn = False
    player2_win_turn = False
    if w == 1:
        ...
        for first_id, first_card_name in enumerate(sorted_name_list):
            for second_id, second_card_name in enumerate(sorted_name_list, start=first_id+1):
                if (first_card_name, second_card_name) in pairwise_dict[f.players[i].name]:
                    target_pair_wise = pairwise_dict[f.players[i].name][(
...
def execute_demo(Player_1, Player_2, iteration, virtual_flg=False, deck_type=None, graph=False):
    ...
    for i, d in enumerate(D):
        if deck_type[i] in key_2_tsv_name:
            ...
            D[i].set_deck_type(
                Embedd_Network_model.deck_id_2_deck_type(deck_type[i]))
            continue
    ...
    for i in range(iteration):
        ...
        if (i + 1) % span == 0:
            ...
            mylogger.info("epoc_win_rate:{:.3%}".format(
                (win_lose[0] - epoc_win_lose[0]) / (epoc_len)))
    ...
    if win_lose[1] == 0:
        win_lose[1] = 1
    mylogger.info("mean_win_turn:{:.3f},{:.3f}".format(
        win_turns[0] / win_lose[0], win_turns[1] / win_lose[1]))
        ...
        for key in list(deck_name_list[player_key].keys()):
            ...
            if target_cell["used_num"] > 0:
                ...
                contribution_list[player_key].append(
                    (key, target_cell["win_num"] / target_cell["used_num"]))
...
def execute_demo_with_pairwise(Player_1, Player_2, iteration, virtual_flg=False,
    ...
    for i, player_name in enumerate(["Alice", "Bob"]):
        ...
        for first_id, first_key in enumerate(card_name_keys):
            for second_id, second_key in enumerate(card_name_keys[first_id+1:], start=first_id+1):
                pairwise_dict[player_name][(first_key, second_key)] = {
                    "both_played_num": 0, "win_num_when_both_played": 0}
    ...
    for i, player_key in enumerate(list(deck_contents.keys())):
        for key in list(deck_contents[player_key].keys()):
            ...
            if target_cell["used_num"] > 0:
                ...
                single_contribution_list[player_key].append(
                    (key, win_rate, played_num))
    for i, player_key in enumerate(list(pairwise_dict.keys())):
        ...
        for key in list(pairwise_dict[player_key].keys()):
            ...
            if target_cell["both_played_num"] > 0:
                ...
                win_rate = target_cell["win_num_when_both_played"] / \
                    target_cell["both_played_num"]
                # mylogger.info("{}'s pairwise WRP(both_played_num:{}):{:.3f}".format(key, both_played_num,win_rate))
                contribution_list[player_key].append(
                    (key, win_rate, both_played_num))
                if both_played_num > (iteration // 10):
                    footcut_contribution_list[player_key].append(
                        (key, win_rate, both_played_num))

                    first_cell = list(
                        filter(lambda x: x[0] == key[0], single_contribution_list[player_key]))[0]
                    second_cell = list(
                        filter(lambda x: x[0] == key[1], single_contribution_list[player_key]))[0]
                    first_single_win_rate = first_cell[1]
                    second_single_win_rate = second_cell[1]
                    pairwise_contribution = win_rate
                    persentages = (first_single_win_rate,
                                   second_single_win_rate)
                    ...
                    pairwise_contribution_list[player_key].append(
                        (key, pairwise_contribution, persentages))
    ...
    for i, player_key in enumerate(list(pairwise_contribution_list.keys())):
        ...
        for j, cell in enumerate(pairwise_contribution_list[player_key]):
            ...
            mylogger.info("{:<80}:{:.3%} | {:.3%} {:.3%}".format(
                txt, cell[1], cell[2][0], cell[2][1]))
    ...
    if output:
        ...
        with open(path, mode="w") as f:
            ...
            for i, player_key in enumerate(list(contribution_list.keys())):
                f.write("Player{}\n".format(i + 1))
                for j, cell in enumerate(contribution_list[player_key]):
                    pairwise = "No.{} ({},{})(both_played_num:{})".format(
                        j + 1, cell[0][0], cell[0][1], cell[2])
                    txt = "{:<80}:{:.3%}\n".format(pairwise, cell[1])
                    f.write(txt)
                f.write("\n")
            f.write("\n")
            f.write(
                "pairwise-WRP_rank(footcut by threthold:{})\n".format(iteration//10))
            for i, player_key in enumerate(list(footcut_contribution_list.keys())):
                f.write("Player{}\n".format(i + 1))
                for j, cell in enumerate(footcut_contribution_list[player_key]):
                    pairwise = "No.{} ({},{})(both_played_num:{})".format(
        ...
        with open(path, mode="w") as w:
            ...
            for i, player_key in enumerate(list(pairwise_contribution_list.keys())):
                writer.writerow(["Player{}".format(
                    i + 1), "first", "second", "pairwise-WRP", "first_WRP", "second_WRP"])
                for j, cell in enumerate(pairwise_contribution_list[player_key]):
                    row = []
...
def get_contributions(Player_1, Player_2, iteration, player1_deck_num=None, directory_name=None):
    ...
    assert player1_deck_num < len(
        D), "player1_deck_num:{}".format(player1_deck_num)
    class_pool = [0, 0]
    ...
    for deck_id in range(len(D)):
        ...
        if player1_deck_num != deck_id:
            file_name = "contribution_{}_vs_{}.txt".format(
                deck_id_2_name[player1_deck_num], deck_id_2_name[deck_id])
        else:
            file_name = "contribution_{}_mirror.txt".format(
                deck_id_2_name[deck_id])
...
def get_basic_contributions(Player_1, Player_2, iteration, virtual_flg=False, player1_deck_num=None,
    ...
    for deck_id in range(len(D)):
        ...
        first_num = [0, 0]
        deck_name_list = {"Alice": current_decks[0].get_name_set(
        ), "Bob": current_decks[1].get_name_set()}
        for i in range(iteration):
            Turn_Players[i % 2].is_first = True
            Turn_Players[i % 2].player_num = 0
            Turn_Players[(i + 1) % 2].is_first = False
        ...
        title += "\n"
        if player1_deck_num != deck_id:
            file_name = "{}_vs_{}'s_WRP_and_WRD.txt".format(
                deck_id_2_name[player1_deck_num], deck_id_2_name[deck_id])
        else:
            file_name = "{}_mirror_WRP_and_WRD.txt".format(
                deck_id_2_name[deck_id])
        with open(directory_name + "/" + file_name, mode="w") as f:
...
def make_deck_table(Player_1, Player_2, iteration, same_flg=False, result_name=None, basic=False, deck_lists=None):
    mylogger.info("{} vs {}".format(
        Player_1.policy.name, Player_2.policy.name))

    if result_name is None:
        result_name = "{}_vs_{}_{}iteration(deck_list={}).tsv".format(
            Player_1.policy.name, Player_2.policy.name, iteration, deck_lists)
    # assert Player1 != Player2
    # Player1.deck = None
    # Player2.deck = None
    win = 0
    ...
    if not basic:
        ...
        #        continue
        for i in deck_lists:
            mylogger.info("{}(deck_id:{}):{}".format(
                deck_index, i, key_2_tsv_name[i]))
    ...
    for j in range(len(D)):
        ...
        for k in range(start, len(D)):
            mylogger.info("{} vs {}".format(
                deck_id_2_name[deck_lists[j]], deck_id_2_name[deck_lists[k]]))
            Turn_Players = [Player1, Player2]
            assert Player1 != Player2
            win_lose = [win, lose]
            first_num = 0
            ...
            mylogger.info("win_rate:{:%} first_win_rate:{:%}".format(
                Results[(j, k)][0], Results[(j, k)][1]))
...
def make_policy_table(n, initial_players=None, deck_type=None, same_flg=False, result_name="Policy_table_result.tsv"):
    ...
    key_2_tsv_name = {0: ["Sword_Aggro.tsv", "SWORD"], 1: ["Rune_Earth.tsv", "RUNE"], 2: ["Sword.tsv", "SWORD"],
                      ...
                      13: ["Neutral_Blood.tsv", "BLOOD"]}
    mylogger.info("{} vs {}".format(
        deck_id_2_name[deck_type[0]], deck_id_2_name[deck_type[1]]))
    policy_id_2_name = {}
        ...
        for policy2_id in range(0, last_id):
            ...
            for i in range(iteration):
                ...
                (win_lose[i % 2], win_lose[(i + 1) % 2], lib_num, end_turn, first, _) = game_play(Turn_Players[i % 2],
                                                                                                  ...
                                                                                                  virtual_flg=True)
                first_num += first
            Results[(policy1_id, policy2_id)] = [
                win_lose[0] / iteration, first_num / iteration]
    ...
    with open("Battle_Result/" + result_name, "w") as f:
        ...
        row = ["{} vs {}".format(
            deck_id_2_name[deck_type[0]], deck_id_2_name[deck_type[1]])]
        mylogger.info("row:{}".format(row))
        mylogger.info("row:{}".format([policy_id_2_name[i]
...
def make_mirror_match_table(Player_1, Player_2, iteration, deck_lists=None, pairwise=False, out_put=False):
    ...
    for deck_id in deck_lists:
        if pairwise:
            ...
            execute_demo(Player_1, Player_2, iteration, virtual_flg=True, deck_type=[
                         deck_id, deck_id], graph=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--mode', help='実行モード、demoで対戦画面表示,policyでdecktype固定で各AIタイプの組み合わせで対戦')
    ...
    # 1
    Players.append(Player(9, True, policy=AggroPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 2
    Players.append(Player(9, True, policy=GreedyPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 3
    Players.append(Player(9, True, policy=FastGreedyPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 4
    Players.append(Player(9, True, policy=MCTSPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 5
    Players.append(Player(9, True, policy=Aggro_MCTSPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 6
    Players.append(Player(9, True, policy=New_Aggro_MCTSPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 7
    Players.append(Player(9, True, policy=Information_Set_MCTSPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 8
    Players.append(Player(9, True, policy=Flexible_Iteration_MCTSPolicy(
        N=step_num), mulligan=Min_cost_mulligan_policy()))  # 9
    Players.append(Player(9, True, policy=Flexible_Iteration_Aggro_MCTSPolicy(
        N=step_num), mulligan=Min_cost_mulligan_policy()))  # 10
    Players.append(Player(9, True, policy=Flexible_Iteration_Information_Set_MCTSPolicy(
        N=step_num), mulligan=Min_cost_mulligan_policy()))  # 11
    Players.append(Player(9, True, policy=Opponent_Modeling_MCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 12
    Players.append(Player(9, True, policy=Opponent_Modeling_ISMCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 13
    Players.append(Player(9, True, policy=Default_GreedyPolicy(),
                   mulligan=Simple_mulligan_policy()))  # 14
    Players.append(Player(9, True, policy=Default_Aggro_MCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 15
    Players.append(Player(9, True, policy=Non_Rollout_A_MCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 16
    Players.append(Player(9, True, policy=Non_Rollout_ISMCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 17
    Players.append(Player(9, True, policy=Non_Rollout_OM_MCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 18
    Players.append(Player(9, True, policy=Non_Rollout_OM_ISMCTSPolicy(
        iteration=step_num), mulligan=Min_cost_mulligan_policy()))  # 19
    Players.append(Player(9, True, policy=Simple_value_function_A_MCTSPolicy(
    ), mulligan=Min_cost_mulligan_policy()))  # 20
    Players.append(Player(9, True, policy=Simple_value_function_ISMCTSPolicy(
    ), mulligan=Min_cost_mulligan_policy()))  # 21
    Players.append(Player(9, True, policy=Simple_value_function_OM_MCTSPolicy(
    ), mulligan=Min_cost_mulligan_policy()))  # 22
    Players.append(Player(9, True, policy=Simple_value_function_OM_ISMCTSPolicy(
    ), mulligan=Min_cost_mulligan_policy()))  # 23
    Players.append(Player(9, True, policy=New_GreedyPolicy(),
                   mulligan=Simple_mulligan_policy()))  # 24
    Players.append(Player(9, True, policy=until_game_end_MCTSPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 25
    Players.append(Player(9, True, policy=until_game_end_OM_MCTSPolicy(
    ), mulligan=Min_cost_mulligan_policy()))  # 26
    Players.append(Player(9, True, policy=Cheating_MO_ISMCTSPolicy(),
                   mulligan=Min_cost_mulligan_policy()))  # 27
    ...
    if args.model_name is not None:
        ...
        if args.model_name == "ini":
            ...
            origin_model = Embedd_Network_model.New_Dual_Net(
                args.node_num, hidden_num=hidden_num)
            Players.append(
    ...
    input_players = [Players[0], Players[1], Players[4],
                     Players[8], Players[9], Players[11], Players[13], Players[14]]
    deck_list = None
    ...
    if args.mode == 'demo' or args.mode == 'background_demo':
        ...
        if args.pairwise is not None:
            if args.cProfile is not None:
                ...
                execute_demo_with_pairwise(d1, d2, n, deck_type=[
                                           p1, p2], virtual_flg=virtual_flg)
        else:
            if args.cProfile is not None:
                cProfile.run('execute_demo(d1, d2, n, deck_type=[p1, p2],virtual_flg = virtual_flg)', sort="tottime",
                             filename="profiling.stats")
            else:
                execute_demo(d1, d2, n, deck_type=[
                             p1, p2], virtual_flg=virtual_flg, graph=graph)
    ...
    # elif sys.argv[-1]=="-policy":
        ...
        n = int(args.N)
        assert args.decktype1 is not None and args.decktype2 is not None, "deck1:{},deck2:{}".format(
            args.decktype1, args.decktype2)
        ...
        if file_name is not None:
            ...
            make_policy_table(n, initial_players=input_players,
                              deck_type=[a, b], same_flg=a == b)
        ...
        d1 = copy.deepcopy(Players[a])
        d2 = copy.deepcopy(Players[b])
        get_contributions(d1, d2, iteration, virtual_flg=True,
                          player1_deck_num=player1_deck_id, directory_name=file_name)
        ...
        path = "{}_vs_{}(basic_all)_{}times_WRP_and_WRD".format(
            short_name_1, short_name_2, iteration)
        os.makedirs(path)
        for i in range(8):
            player1_deck_id = i
        ...
        short_name_1 = list(d1.policy.name.split("Policy"))[0]
        short_name_2 = list(d2.policy.name.split("Policy"))[0]
        path = "{}_vs_{}(custom_all)_{}times_WRP_and_WRD".format(
            short_name_1, short_name_2, iteration)
        ...
        for i in range(10):
            ...
            if os.path.isdir(path + next_path):
                mylogger.info(
                    "{} already exists. skip this step.".format(next_path))
                continue
            os.makedirs(path + next_path)
            file_name = path + next_path
        ...
        if args.N is not None:
            ...
            if False:
                ...
                make_deck_table(d1, d2, iteration, result_name=file_name,
                                basic=basic_flg, deck_lists=deck_list)
    else:
        assert False
    mylogger.info(t1)
