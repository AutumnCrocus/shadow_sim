...
class Simple_State_Net(nn.Module):
    def __init__(self, n_mid,rand=False):
        ...
        hidden_layer_num = 3
        origin = 105*self.vec_size#94*self.short_mid
        ...
        modify_layer = [Dual_ResNet(node_size_list[i], node_size_list[i+1],activate=nn.PReLU(init=0.01)) for i in range(hidden_layer_num)]
    ...
    def forward(self, states):
        ...
        class_values = class_values.expand(-1, 16, self.vec_size)#.expand(-1, 16, self.n_mid)
        deck_type_values = self.deck_type_eye[deck_type_datas].view(-1, 8).unsqueeze(-1)#.to(stats.device)
        deck_type_values = deck_type_values.expand(-1, 8, self.vec_size)#.expand(-1, 8, self.n_mid)
