...
class Player:
    ...
    def decide(self, player, opponent, field, virtual=False):
        ...
        while True:
            policy_count += 1
            if policy_count > 20:
                mylogger.info("action_cdoe:{}".format((action_num, card_id, target_id)))
                self.show_hand()
                field.show_field()
                ...
                if len(real_hand) != len(sim_hand):
                    ...
                    if action_num==Action_Code.PLAY_CARD.value:
                        if card_id >= len(self.hand):
                                re_check=True
                                msg="illigal play card error"
                        elif sim_field.players[self.player_num].hand[card_id].name != self.hand[card_id].name:
                            msg = "diff card name error({} is not {})".\
                                format(sim_field.players[self.player_num].hand[card_id].name,self.hand[card_id].name)
                            re_check = True
                    if not re_check:
                        if any(field.card_num[i] != sim_field.card_num[i] for i in range(2)):
                            re_check = True
                            msg = "diff field len error"
                        if not re_check:
                            for side_id in range(2):
                                for location_id in range(len(field.card_location[side_id])):
                                    if field.card_location[side_id][location_id].name != \
                                            sim_field.card_location[side_id][location_id].name:
            ...
            if action_num == -1:
                ...
                if card_id not in regal_targets:
                    ...
                    re_check = True
                elif self.hand[card_id].have_target == 8 and (self.policy.policy_type == 3):
                    real_hand = self.hand
                    sim_hand = self.policy.current_node.field.players[self.player_num].hand
                    if len(real_hand) != len(sim_hand):
            ...
            if re_check == True and self.policy.policy_type == 3:
                if not virtual:
                    mylogger.info(msg)
                    """
                    if msg!="diff hand len error" and msg != "diff field len error" and msg != "diff field card error":
                        sim_field = self.policy.current_node.field
                        field.show_field()
